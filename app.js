...
document.addEventListener('DOMContentLoaded', function() {
  ...
  // These will be set after injection
  let manualStartCheck, fixedStartInput;
  ...
  function renderGantt() {
    ...
    activities.forEach((act, index) => {
      ...
      rect.setAttribute('fill', act.manualStart ? '#f59e0b' : '#3b82f6');
  ...
  function ensureModalFields() {
    ...
    if (document.getElementById('manualStartCheck')) {
      manualStartCheck = document.getElementById('manualStartCheck');
      fixedStartInput = document.getElementById('fixedStartInput');
      return;
    }

    const form = document.getElementById('activityForm');
    // Find the container that holds the predecessor section (the div directly containing the label, list, and add row)
    const predContainer = document.getElementById('predecessorList').parentNode; // this is the div wrapping the whole predecessor block
    ...
    // Insert the new block right before the predecessor container
    form.insertBefore(manualDiv, predContainer);
