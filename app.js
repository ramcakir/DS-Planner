...
function calculateEarlyDates() {
  ...
  do {
    ...
    activities.forEach(act => {
      if (act.type === 'Milestone' && act.duration === 0) act.duration = 0; // ensure
...
addPredBtn.addEventListener('click', () => {
  const id = newPredId.value.trim();
  const type = newPredType.value;
  if (!id) return;
  // check if activity exists? we can allow, but warn later
  if (!activities.some(a => a.id == id) && id !== '') {
...
activityForm.addEventListener('submit', (e) => {
  ...
  const duration = parseInt(editDuration.value, 10) || 0;
  ...
  if (id) {
    ...
    if (act) {
      ...
      act.predecessors = currentPreds.filter(p => p.id.trim() !== '');
    }
  } else {
    // create new
    const newId = nextId++;
    activities.push({
      ...
      type,
      category,
      predecessors: currentPreds.filter(p => p.id.trim() !== '')
    });
  }
    ...
    try {
      ...
      if (data.activities) {
        activities = data.activities.map(a => {
          // ensure predecessors array exists
          if (!a.predecessors) a.predecessors = [];
          return a;
        });
        nextId = data.nextId || Math.max(...activities.map(a => a.id), 0) + 1;
